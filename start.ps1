...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$clientPath'; Write-Host '=== React Client Starting on port 3000 ===' -ForegroundColor Green; Write-Host ''; `$env:HOST='0.0.0.0'; npm start" -WindowStyle Normal

Write-Host ""
Write-Host "✅ Server: http://localhost:5000" -ForegroundColor Green
Write-Host "✅ Client: http://localhost:3000" -ForegroundColor Green
Write-Host "✅ Mobile: http://192.168.0.104:3000" -ForegroundColor Green
Write-Host ""
Write-Host "Two PowerShell windows have been opened:" -ForegroundColor Yellow
Write-Host "- Chat Server (port 5000)" -ForegroundColor Yellow
Write-Host "- Chat Client (port 3000)" -ForegroundColor Yellow
Write-Host ""
Write-Host "To stop: Run stop.ps1 or stop.bat" -ForegroundColor Cyan
Write-Host ""
